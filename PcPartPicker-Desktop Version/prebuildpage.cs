    ...
    public partial class prebuildpage : UserControl
    {
        ...
        private void prebuildpage_Load(object sender, EventArgs e)
        {
            int id = Convert.ToInt32(prebuild.sID);
              ...
              //BUILD
              var bd = from a in db.BUILD where a.Build_ID == id select a ;
            ...
            int userid = Convert.ToInt32(this.label2.Text);
              //user 
              var us = from a in db.USER where a.USER_ID == userid select a;
            ...
            // cpu info
            var cp = from a in db.Cpu where a.Cpu_ID == cpuname.Text select a;
            dgcpu.DataSource = cp;
            ...
            // motherboard info 
            var mb = from a in db.MotherBoard where a.MoBo_ID == moboname.Text select a;
            ...
            // ram info 
            var rm = from a in db.Memory where a.Memory_ID == memoryname.Text select a;
            dgram.DataSource = rm;
            ...
            // storage info
            var st = from a in db.Storage where a.Storage_ID == storagename.Text select a;
            ...
            // gpu info 
            var gp = from a in db.Gpu where a.Gpu_ID == gpuname.Text select a;
            dggpu.DataSource = gp;
            ...
            // case info 
            var cs = from a in db.Case where a.Case_ID == casename.Text select a;
            ...
            // PSU info 
            var ps = from a in db.PowerSupply where a.PowerSupply_ID == psuname.Text select a;
            dgpsu.DataSource = ps;
            ...
            // cpu cooler info 
            var cpp = from a in db.CpuCooler where a.CpuCooler_ID == cpucoolername.Text select a;
